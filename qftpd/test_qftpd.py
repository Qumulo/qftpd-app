...
class TestWriteBuffer(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        local_rc = get_rc()
        target_name = os.path.join('/', test_file_name)
        print "target_name: %s" % target_name
        try:
            local_rc.fs.delete(target_name)
    ...
    def test_write_buffer_close_writes_qsfs(self):
        ...
        write_buffer.write(test_file_contents)
        write_buffer.close()
        sleep(5)  # values below 5 result in intermittent failures
...
class TestQFTPAuthentication(unittest.TestCase):
    ...
    # @mock.patch('qftpd.pyftpdlib')
    # def test_handler_run_as_current_user(self):
    #     """instantiate a handler, call run_as_current_user()
    #     make sure the filesystem instance's self.rc attribute ends up with a logged-in RestClient"""
    #     authorizer = qftpd.QSFSAuthorizer()
    #     handler = qftpd.QFTPHandler
    #     handler.authorizer = authorizer
    #     handler.abstracted_fs = qftpd.AbstractedQSFS
    #     h = handler(None, None)
    #     h.run_as_current_user('admin', 'a')
    #     self.assertIs(type(h.fs.rc), RestClient)


class TestQumuloRest(unittest.TestCase):
    def test_big_file_write(self):
        rc = get_rc()
        rc.fs.create_file('big_test_file', '/')
        big_test_file = open('big_testfile', 'r')
        rc.fs.write_file(big_test_file, '/big_test_file')
        self.assertFalse(True)

    def test_tiny_file_write(self):
        rc = get_rc()
        testfile_contents = "foo"
        tiny_file = StringIO(testfile_contents)
        for i in range (10000):
            test_file_name = 'test_file_' + str(i)
            rc.fs.create_file(test_file_name, '/')
            rc.fs.write_file(tiny_file, '/' + test_file_name)
        self.assertFalse(True)
